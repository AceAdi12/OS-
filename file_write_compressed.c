...
int main(int argc, char** argv){
    if(argc!=4){
        fprintf(stderr, RED "Usage: %s <virtual_disk_meta> <storage_pool> <input_file>\n" RESET, argv[0]);
        return 1;
    }
    const char* meta = argv[1];
    ...
    if (hash_lookup("hash_index.txt", hex, &dupe_off, &dupe_sz)){
        printf(YELLOW "⚠️  Duplicate detected — reusing existing blob at offset %lld (%lld bytes)\n" RESET, dupe_off, dupe_sz);
